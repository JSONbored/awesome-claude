...
export function newsletterApiErrorMessage(data: unknown, fallback: string) {
...
export async function subscribeToNewsletter(
  ...
  try {
    ...
    if (!res.ok) {
      return {
        ok: false,
        error: newsletterApiErrorMessage(data, `Subscribe failed (${res.status}).`),
      };
    }
...
export async function unsubscribeFromNewsletter(input: {
  ...
  try {
    ...
    if (!res.ok) {
      return {
        ok: false,
        error: newsletterApiErrorMessage(data, `Unsubscribe failed (${res.status}).`),
      };
    }
    return { ok: true };
  } catch {
