...
This is a **user-written [Stop hook](https://code.claude.com/docs/en/hooks)** — a Bash script that Claude Code's hooks system runs automatically when your session ends. Claude Code's hooks documentation describes the `Stop` trigger and the `script` configuration field used here. This particular recipe detects your project's package manager from its lockfile and runs the appropriate vulnerability scanner, writing a timestamped report to your project root.

**What the hooks docs cover that this hook relies on:**
- The `Stop` trigger fires after Claude Code finishes each session
- Hooks are configured via the `hooks.stop.script` field in `.claude/settings.json`, `.claude/settings.local.json`, or `~/.claude/settings.json`
- The `script` path is executed as a shell command from the project root

**Package manager detection order (based on lockfile presence):**
...
Each run writes a timestamped `security-audit-YYYYMMDD_HHMMSS.log` in the project root. Add `security-audit-*.log` to `.gitignore` to keep reports local.

## Installation

Following the hooks documentation, configure this hook by saving the script and registering it in your settings:
...
3. Register it under `hooks.stop` in one of the three settings paths the hooks docs describe

**Settings file locations (from the hooks docs):**
...
- Claude Code CLI (provides the Stop hook trigger and settings.json configuration)
- Bash shell
- At least one supported audit tool for your stack:
  - **Node.js**: `npm` (built in to npm 6+) or `yarn` (any version with `yarn audit`)
...
### Audit logs accumulate in the project root

Change the `REPORT_FILE` path in the script to a subdirectory, e.g. `.claude/logs/security-audit-$TIMESTAMP.log`. Add `security-audit-*.log` to `.gitignore` to exclude reports from version control.

### npm audit hangs on network issues

Wrap the call with a timeout: `timeout 60 npm audit`. Use `npm audit --offline` for cached advisory data. Set a registry timeout with `npm config set timeout 30000`.

### Safety scanner fails with "database not found"

Run `safety check --update-db` to refresh the local advisory database, or upgrade: `pip install --upgrade safety`. Alternatively, use `pip-audit` (`pip install pip-audit`) and update the Python branch of the script.

### Audit severity flag not recognised by older npm

The `--audit-level` flag requires npm 6+. For earlier versions, remove the flag and parse severity from the full output with `grep`. Check your version with `npm --version`.

### bundler-audit requires bundle install first

Run `bundle install` before the audit: `bundle install && bundle exec bundler-audit check`. Ensure `Gemfile.lock` is current and `bundler-audit` is installed globally or in the bundle.
