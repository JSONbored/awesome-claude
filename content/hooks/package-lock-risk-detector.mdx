---
title: Package Lock Risk Detector - Claude Code Hook
slug: package-lock-risk-detector
category: hooks
description: >-
  PostToolUse hook that inspects an edited npm package-lock.json for
  supply-chain provenance risks rather than known CVEs — dependencies resolved
  from a non-registry source, packages fetched over insecure http, and registry
  tarballs missing an integrity hash. It mirrors the lockfile-lint checks so a
  tampered or unexpected dependency source is caught at edit time. Advisory
  only; it never installs or runs anything.
cardDescription: >-
  Flag npm lockfile entries with non-registry sources, insecure http, or missing
  integrity hashes.
seoTitle: Package Lock Risk Detector - Claude Code Hook
seoDescription: >-
  A Claude Code PostToolUse hook that flags npm package-lock.json provenance
  risks — non-registry sources, insecure http, and missing integrity — using the
...
installCommand: >-
  ...
  .claude/hooks/package-lock-risk-detector.sh && chmod +x
  .claude/hooks/package-lock-risk-detector.sh
usageSnippet: "Dependencies resolved from a non-registry source: node_modules/foo: git+https://..."
copySnippet: |-
  {
    "hooks": {
      "PostToolUse": [
        {
          ...
          "hooks": [
            {
              "type": "command",
              "command": "$CLAUDE_PROJECT_DIR/.claude/hooks/package-lock-risk-detector.sh"
            }
          ]
        }
      ]
...
scriptBody: |-
  ...
  # resolved from a non-registry source, insecure http resolves, and registry
  # tarballs missing an integrity hash. Advisory only - it always exits 0,
  # never installs or runs anything - and fails open when jq is unavailable.
  ...
  report "Dependencies resolved from a non-registry source (supply-chain risk):" \
    '.packages | to_entries[] | select(.value.resolved) | select(.value.resolved | test("^https://registry\\.npmjs\\.org/") | not) | "\(.key): \(.value.resolved)"'

  report "Dependencies fetched over insecure http (tamperable in transit):" \
    '.packages | to_entries[] | select(.value.resolved) | select(.value.resolved | test("^http://")) | "\(.key): \(.value.resolved)"'
...
safetyNotes:
  ...
  - "Uses the resolved-URL and integrity fields to flag provenance risk (non-registry host, insecure http, missing integrity); it does not assess known vulnerabilities, so pair it with an audit tool."
privacyNotes:
...
- Detection mirrors the [lockfile-lint](https://github.com/lirantal/lockfile-lint) check set: dependencies resolved from a **non-registry source** (git, http tarball, or an alternate registry), packages fetched over **insecure http**, and registry tarballs **missing an integrity hash**.
...
On `PostToolUse`, the hook checks whether the edited file is an npm lockfile. For `package-lock.json` (npm v2/v3, the `.packages` map), it scans each resolved dependency and reports any entry whose `resolved` URL is not the public npm registry, uses `http://`, or lacks an `integrity` hash. Findings go to stderr with a reminder to confirm the sources and run lockfile-lint for the full check set.

## Why provenance, not CVEs

CVE scanners answer "does this version have a known vulnerability?". This hook answers a different question: "did this dependency come from where I expect?" A swapped registry, a git URL pointing at a fork, an http tarball, or a missing integrity hash are tampering and dependency-confusion signals that a CVE scan will not catch.

## Use cases

- Catch a dependency-confusion or registry-swap edit the moment the lockfile changes.
- Enforce "registry + integrity only" provenance locally, ahead of a full lockfile-lint run in CI.
- Surface a stray git or http dependency introduced during an agentic edit.

## Installation

1. Create the hooks directory: `mkdir -p .claude/hooks`
2. Create the hook file: `touch .claude/hooks/package-lock-risk-detector.sh`
3. Paste the script body into that file and make it executable: `chmod +x .claude/hooks/package-lock-risk-detector.sh`
