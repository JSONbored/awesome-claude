...
category: hooks
description: "A Claude Code SessionEnd hook that scans a project for dead code and writes a report to .claude/reports. It runs available tools per language: ESLint and ts-prune for JS/TS, autoflake or pylint for Python, depcheck for unused npm dependencies, ripgrep for unreferenced src files, and jq to flag zero-coverage files. Defaults to DRY_RUN=true so nothing is deleted; it only analyzes and reports."
cardDescription: "SessionEnd Bash hook that reports dead code via ts-prune, ESLint, autoflake, and depcheck. Dry-run by default; writes findings to .claude/reports."
seoTitle: "SessionEnd Dead Code Report Hook for Claude Code"
seoDescription: "Claude Code hook that runs ts-prune, ESLint, autoflake, and depcheck at session end, reporting unused code and dependencies to a file. Dry-run by default."
author: JSONbored
