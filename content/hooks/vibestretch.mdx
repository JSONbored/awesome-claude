...
privacyNotes:
  ...
  - >-
    Reads the first 4 KB of the hook payload to extract session_id. Nothing from
    prompts or tool output is retained or transmitted; only timestamps and
    elapsed seconds are stored.
  - >-
...
and unknown terminals get nothing rather than stray bytes. VS Code's integrated
terminal has no desktop notification of its own and discards the sequence
silently, so there the banner appears only if you add an extension that turns
OSC notifications into native ones. The chime and the status line are the
channels that do not depend on any of this.
