...
category: hooks
description: >-
  PostToolUse hook that intercepts writes to migration, schema, and SQL files,
  auto-detects your ORM from project files, and surfaces pending-migration
  status plus DROP/DELETE/TRUNCATE warnings directly in Claude Code output.
cardDescription: >-
  Detects migration and SQL file edits, checks package.json for
  Knex/Sequelize/TypeORM, looks for manage.py or a Rails Gemfile, and prints
  pending-migration counts plus safe-practice reminders to stderr...
seoTitle: Database Migration Runner - Hooks
seoDescription: >-
  Claude Code PostToolUse hook triggered by writes or edits to files matching
  migration, schema, or .sql patterns. Checks package.json for Knex 3.x,
  Sequelize 6.x/7.x, or TypeORM 0.3.x; runs showmigrations for Django 5.x
  projects; inspects raw SQL for destructive statements before surfacing
  actionable status in the session. Discover tools for AI development.
author: JSONbored
