...
- The root README points to official Claude Code documentation at
  `code.claude.com/docs/en/overview` for launch messaging accuracy.
- The README documents recommended install methods (curl, Homebrew, WinGet)
  suitable for internal launch communications.
- The README directs users to report issues via `/bug` or GitHub issues at
  `anthropics/claude-code` for launch support channels.
- `plugins/README.md` catalogs official plugins (for example `feature-dev` and
  `code-review`) that launch campaigns can reference as concrete examples.
- `CHANGELOG.md` combines separate MCP and connector authentication startup
  notifications into a single message, relevant for rollout comms about first-run
  experience.
