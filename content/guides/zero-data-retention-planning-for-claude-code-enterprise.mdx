...
- The root README "Data collection, usage, and retention" section links to official
  data usage policies and describes feedback collection including conversation data.
- The README documents privacy safeguards: limited retention periods, restricted
  access to session data, and policies against using feedback for model training.
- The README references Commercial Terms of Service and Privacy Policy for full
  enterprise retention details.
- `CHANGELOG.md` documents `--safe-mode` to disable plugins, skills, hooks, and
  MCP servers—relevant when auditing which surfaces remain under ZDR scope.
- `plugins/README.md` shows plugins may ship `.mcp.json` external integrations
  that require separate third-party retention review outside Anthropic policies.
