...
- `CHANGELOG.md` documents `/usage` per-category breakdown of limits usage,
  including skills, subagents, plugins, and per-MCP-server cost.
- `CHANGELOG.md` records fast mode on Opus 4.8 at 2x the standard rate for 2.5x
  speed, relevant for team cost forecasting.
- `CHANGELOG.md` documents VSCode usage attribution showing cache misses, long
  context, subagents, and plugin/MCP breakdowns.
- The root README links to Commercial Terms of Service and data usage policies
  governing billing and retention for enterprise accounts.
- `CHANGELOG.md` records automatic compaction when 1M-context sessions exceed
  credit limits, preventing runaway context-related spend.
