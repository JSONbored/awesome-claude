...
- The root README documents enterprise-friendly install paths including curl,
  Homebrew, and WinGet rather than deprecated global npm install.
- `CHANGELOG.md` fixes remote MCP server connection failures in Claude Code
  Remote sessions when an egress proxy is enabled.
- `CHANGELOG.md` documents managed-settings MCP predicates using `${VAR}`
  references, relevant to proxy and certificate environment configuration.
- `CHANGELOG.md` records stdio MCP servers receiving `CLAUDE_CODE_SESSION_ID`
  and `CLAUDECODE=1` in their environment alongside hooks and Bash.
- `CHANGELOG.md` documents enterprise managed MCP policies
  (`allowedMcpServers` / `deniedMcpServers`) enforced across reconnect paths.
