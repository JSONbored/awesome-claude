...
- `plugins/README.md` includes `explanatory-output-style` and
  `learning-output-style` plugins that inject non-coding behavior via
  SessionStart hooks.
- The `explanatory-output-style` plugin README notes it mimics the deprecated
  Explanatory output style through session-start context injection.
- `CHANGELOG.md` documents `/theme` custom theme and color editor dialogs,
  relevant when output styles interact with terminal presentation.
- `CHANGELOG.md` records fixes for `/config` exit summaries incorrectly
  reporting theme changes alongside unrelated setting toggles.
- The root README positions Claude Code as an agentic tool for terminal, IDE,
  and GitHub workflows—not only traditional coding tasks.
