...
5. **Wire the orchestrator.** Point Claude Code at the HTTP endpoints: `claude mcp add --transport http <name> http://YOUR_HOST:<port>/mcp`. Note the config must use `"type": "http"` — bare `"url"` entries are silently ignored.
