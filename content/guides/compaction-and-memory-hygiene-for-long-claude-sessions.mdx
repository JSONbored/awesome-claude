...
- `CHANGELOG.md` documents automatic compaction when sessions use 1M context
  without usage credits, returning under the standard context limit.
- `CHANGELOG.md` records that the "CLAUDE.md is too long" warning threshold
  scales with the active model's context window.
- `CHANGELOG.md` fixes background-agent completion notifications that triggered
  premature "out of context" behavior on some 1M-context models.
- `CHANGELOG.md` improves `/feedback` reports to include conversation from
  before context compaction for long-session triage.
- The root README describes Claude Code handling git workflows and routine tasks
  across extended terminal sessions.
