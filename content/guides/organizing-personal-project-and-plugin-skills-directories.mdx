...
- `plugins/README.md` documents an optional `skills/` directory in the standard
  Claude Code plugin layout for Agent Skills.
- The repository includes a `plugin-dev` plugin with skills for hook development,
  MCP integration, plugin structure, and skill development.
- `CHANGELOG.md` records that plugins placed in `.claude/skills` directories are
  automatically loaded without requiring a marketplace entry.
- `CHANGELOG.md` documents a `/reload-skills` command and `SessionStart` hook
  support for `reloadSkills: true` to re-scan skill directories mid-session.
- Official plugins such as `frontend-design` and `hookify` ship skills alongside
  commands and hooks in the documented plugin structure.
