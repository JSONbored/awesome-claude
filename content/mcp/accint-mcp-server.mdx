...
installable: true
installCommand: >-
  curl -fsSL https://raw.githubusercontent.com/maxbaluev/accreted-intelligence/main/bootstrap/install | ACC_INSTALL_REF=gh-heyclaude ACC_INSTALL_SOURCE='ref=gh-heyclaude&utm_source=heyclaude&utm_campaign=mcp-directory' sh
usageSnippet: >-
  Install `acc`, run `acc hosts-sync` for Claude Code wiring, then let Claude
  call `acc_retrieve` for scored Work Model context and `acc_act` for solve,
  continue, exec, register, browser, and outcome actions.
copySnippet: |-
  {
    "mcpServers": {
      "accint": {
...
For macOS or Linux, install with the public installer:
...
```powershell
$env:ACC_INSTALL_REF='gh-heyclaude'; $env:ACC_INSTALL_SOURCE='ref=gh-heyclaude&utm_source=heyclaude&utm_campaign=mcp-directory'; irm https://raw.githubusercontent.com/maxbaluev/accreted-intelligence/main/bootstrap/install.ps1 | iex
```

After install, wire local agent hosts:
