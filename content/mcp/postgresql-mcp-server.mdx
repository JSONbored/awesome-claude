...
prerequisites:
  ...
  - PostgreSQL connection string with user, password, host, port, and database
...
3. Construct your PostgreSQL connection string with user, password, host, port, and database
...
Verify PostgreSQL is running with pg_isready command. Check that the connection string includes the expected user, password, host, port, and database values. Test with psql command first using your real PostgreSQL URI. Verify firewall allows connections on port 5432 (default). Check PostgreSQL is listening on correct interface (localhost vs 0.0.0.0). Verify hostname resolves correctly for remote connections.
