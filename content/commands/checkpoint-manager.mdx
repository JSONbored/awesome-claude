...
> This is a built-in Claude Code feature, not a custom command and not a plugin. There is no `/checkpoint-manager` command, and there are no `--create`, `--list`, `--interval`, or `--auto-checkpoint` flags. Checkpointing is automatic and is driven entirely through the `/rewind` menu.
...
There is no manual "create checkpoint" step and no configurable checkpoint interval — checkpointing is tied to your prompts.
