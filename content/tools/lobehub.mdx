...
installable: true
installCommand: "Review the official setup script, then run Docker Compose from the LobeHub self-hosting guide."
usageSnippet: >-
...
prerequisites:
  ...
  - "A review of the official remote setup script and generated Docker Compose files before running them on a machine that has sensitive data or privileged Docker access."
...
safetyNotes:
  - "The README's Docker quick start uses a remote setup script. Review the script, generated compose files, image tags, mounted volumes, and network bindings before running it on production or personal machines."
...
The README documents Docker Compose as a self-hosting path:
...
Review the setup script and generated compose files before running them. At
minimum, the README's quick-start environment table requires:
...
- The README documents self-hosting with Vercel, Alibaba Cloud, and Docker, plus
  the Docker Compose setup flow and required `OPENAI_API_KEY`.
