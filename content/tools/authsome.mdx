...
privacyNotes:
  ...
  - When the agent calls an external provider, the local proxy injects the corresponding OAuth2 access token or API key into the outbound request and sends it to that provider (e.g. github.com, openai.com, slack.com). Credentials reach the provider you authenticated with, by design, since that is required for the provider call to succeed.
