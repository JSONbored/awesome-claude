...
seoDescription: Authsome is an open-source local credential broker for AI agents with OAuth2 and API key support, an encrypted vault, and a local HTTP proxy that injects credentials so agents never see raw secrets.
author: Agentr
