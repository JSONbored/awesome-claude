...
category: skills
description: >-
  Expert Claude Agent SDK OpenTelemetry observability capability pack for designing, reviewing, and rolling out Agent SDK OpenTelemetry observability with source-backed checklists, production rules, and privacy-safe output contracts.
cardDescription: >-
  Review and operationalize Agent SDK OpenTelemetry observability with source-backed Claude Code checklists.
seoTitle: Claude Agent SDK OpenTelemetry Observability Capability Pack Skill
seoDescription: >-
  Source-backed capability pack for Agent SDK OpenTelemetry observability: workflow steps, review matrix, troubleshooting, and rollout notes aligned to official Claude Code documentation.
author: kiannidev
...
installable: false
usageSnippet: >-
  Use this skill when planning or hardening Agent SDK OpenTelemetry observability in Claude Code, Agent SDK, or MCP integrations.
copySnippet: |-
  # Trigger
  "Apply the Claude Agent SDK OpenTelemetry observability capability pack for this environment."

  # Required output
  1) Agent SDK OpenTelemetry observability scope and current configuration summary
...
testedPlatforms:
  ...
  - Generic AGENTS
prerequisites:
  - "Access to Claude Code or Agent SDK environment where Agent SDK OpenTelemetry observability will run."
...
safetyNotes:
  - "This skill plans Agent SDK OpenTelemetry observability; it must not execute destructive changes without explicit approval."
...
privacyNotes:
  - "Reviews may expose integration tokens, customer metadata, and internal URLs related to Agent SDK OpenTelemetry observability."
...
keywords:
  ...
  - "Claude Code Agent SDK OpenTelemetry observability"
  - "Agent SDK OpenTelemetry observability review checklist"
  - "Agent SDK OpenTelemetry observability production rules"
  - "Claude Code Agent SDK OpenTelemetry observability rollout"
  - "Agent SDK OpenTelemetry observability troubleshooting"
readingTime: 9
difficultyScore: 80
...
This capability pack is grounded in Claude Code agent-sdk/observability, overview, analytics, and security documentation
...
- Official docs for Agent SDK OpenTelemetry observability are published on code.claude.com with settings and security cross-links.
...
This is not vendor professional services. Use it as a reusable review workflow for Agent SDK OpenTelemetry observability on Claude Code and related Agent SDK surfaces.

## Core Workflow

1. Confirm prerequisites, account plan, and Claude Code or SDK version for Agent SDK OpenTelemetry observability.
...
- Agent SDK OpenTelemetry observability scope definition.
...
- **Claude Code / Claude**: use as an Agent Skill when preparing Agent SDK OpenTelemetry observability
...
- **Codex, Cursor, Windsurf, and Generic AGENTS workflows**: use the workflow as a
  deterministic checklist for Agent SDK OpenTelemetry observability evaluations on Claude Code projects.
...
requests for Claude Agent SDK OpenTelemetry observability, Agent SDK OpenTelemetry observability, and Claude Code capability pack workflows. Official docs describe the feature directly, but
