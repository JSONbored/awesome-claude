...
export function buildEntryJsonLd(entry, params = {}) {
  const siteUrl = params.siteUrl || "https://heyclau.de";
  const url = absoluteSiteUrl(siteUrl, `/entry/${entry.category}/${entry.slug}`);
  const label =
...
export function buildEntryJsonLdSnapshot(entry, params = {}) {
  ...
  const breadcrumb = buildBreadcrumbJsonLd([
    ...
    { name: label, url: absoluteSiteUrl(siteUrl, `/browse?category=${encodeURIComponent(entry.category)}`) },
