...
describe("containsClosurePhrase", () => {
  ...
  it("matches case-sensitively against the lowercase phrase list", () => {
    // The check is a plain substring match against lowercase phrases, so an
    // upper-cased variant of the same phrase does not match.
    expect(containsClosurePhrase("this role is closed")).toBe(true);
    expect(containsClosurePhrase("THIS ROLE IS CLOSED")).toBe(false);
  });
});
