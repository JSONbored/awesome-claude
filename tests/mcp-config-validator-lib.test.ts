...
describe("MCP config validator lib", () => {
  describe("redactEnvValue", () => {
    it("redacts secret-like env keys and raw token values", () => {
      expect(
        redactEnvValue("GITHUB_TOKEN", "ghp_abcdefghijklmnopqrstuvwxyz123456"),
      ).toBe("${GITHUB_TOKEN}");
      expect(
        redactEnvValue("owner", "ghp_abcdefghijklmnopqrstuvwxyz123456"),
      ).toBe("${OWNER}");
    ...
    it("preserves placeholders and empty values", () => {
      ...
      expect(redactEnvValue("", "ghp_abcdefghijklmnopqrstuvwxyz123456")).toBe(
        "${SECRET}",
      );
    });
  });
  ...
  describe("redactUrlValue", () => {
    ...
    it("preserves encoded placeholders that are not generated by redaction", () => {
      const result = redactUrlValue(
        "https://evil.example/mcp?x=%24%7BGITHUB_TOKEN%7D&safe=1",
      );
      expect(result.redactedCount).toBe(0);
      expect(result.value).toContain("%24%7BGITHUB_TOKEN%7D");
      expect(result.value).not.toContain("${GITHUB_TOKEN}");
    });
  });

  describe("redactArgValue", () => {
    it("redacts key=value args and bare token prefixes", () => {
      expect(redactArgValue("token=ghp_12345678901234567890")).toEqual({
        value: "token=${TOKEN}",
        redactedCount: 1,
      });
      expect(redactArgValue("gho_abcdefghijklmnopqrstuvwxyz123456")).toEqual({
        value: "${SECRET}",
        redactedCount: 1,
      });
    });
  ...
  describe("validateServer", () => {
    it("classifies stdio and remote transports with env metadata", () => {
      const stdio = validateServer("github", {
        ...
        env: { GITHUB_PERSONAL_ACCESS_TOKEN: "ghp_real_token_value" },
      });
