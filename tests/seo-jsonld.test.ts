...
describe("SEO JSON-LD policy", () => {
  ...
  it("maps entry categories to correct JSON-LD @type via unified buildEntryJsonLd", () => {
    ...
    if (guideEntry) {
      expect(
        buildEntryJsonLd(guideEntry, { siteUrl: "https://heyclau.de" })["@type"],
      ).toBe("TechArticle");
    }
    ...
    for (const category of CODE_LIKE_CATEGORIES) {
      ...
      if (codeEntry) {
        expect(
          buildEntryJsonLd(codeEntry, { siteUrl: "https://heyclau.de" })["@type"],
    ...
    if (creativeEntry) {
      expect(
        buildEntryJsonLd(creativeEntry, { siteUrl: "https://heyclau.de" })["@type"],
      ).toBe("CreativeWork");
    }
  });
