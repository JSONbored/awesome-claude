...
describe("brief-issues-lib isMissingBriefIssuesInfra", () => {
  ...
  it("isMissingBriefIssuesInfra message 1", () => {
    expect(isMissingBriefIssuesInfra(new Error("SQLITE_ERROR: no such table: brief_issues"))).toBe(true);
  });
  it("isMissingBriefIssuesInfra message 2", () => {
    expect(isMissingBriefIssuesInfra(new Error("D1_ERROR: no such table"))).toBe(true);
  });
  it("isMissingBriefIssuesInfra message 3", () => {
    expect(isMissingBriefIssuesInfra(new Error("no such table: other_table"))).toBe(true);
  });
  it("isMissingBriefIssuesInfra message 4", () => {
    expect(isMissingBriefIssuesInfra(new Error("constraint failed"))).toBe(false);
  });
  it("isMissingBriefIssuesInfra message 5", () => {
    expect(isMissingBriefIssuesInfra(new Error("database is locked"))).toBe(false);
  });
...
describe("brief-issues-lib parseBriefIssueRow", () => {
  ...
  it("parseBriefIssueRow status draft 0", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 1, status: "draft", slug: "brief-draft-0" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-0");
  });
  it("parseBriefIssueRow status draft 1", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 2, status: "draft", slug: "brief-draft-1" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-1");
  });
  it("parseBriefIssueRow status draft 2", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 3, status: "draft", slug: "brief-draft-2" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-2");
  });
  it("parseBriefIssueRow status draft 3", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 4, status: "draft", slug: "brief-draft-3" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-3");
  });
  it("parseBriefIssueRow status draft 4", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 5, status: "draft", slug: "brief-draft-4" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-4");
  });
  it("parseBriefIssueRow status draft 5", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 6, status: "draft", slug: "brief-draft-5" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-5");
  });
  it("parseBriefIssueRow status draft 6", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 7, status: "draft", slug: "brief-draft-6" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-6");
  });
  it("parseBriefIssueRow status draft 7", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 8, status: "draft", slug: "brief-draft-7" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-7");
  });
  it("parseBriefIssueRow status draft 8", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 9, status: "draft", slug: "brief-draft-8" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-8");
  });
  it("parseBriefIssueRow status draft 9", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 10, status: "draft", slug: "brief-draft-9" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-9");
  });
  it("parseBriefIssueRow status draft 10", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 11, status: "draft", slug: "brief-draft-10" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-10");
  });
  it("parseBriefIssueRow status draft 11", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 12, status: "draft", slug: "brief-draft-11" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-11");
  });
  it("parseBriefIssueRow status draft 12", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 13, status: "draft", slug: "brief-draft-12" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-12");
  });
  it("parseBriefIssueRow status draft 13", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 14, status: "draft", slug: "brief-draft-13" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-13");
  });
  it("parseBriefIssueRow status draft 14", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 15, status: "draft", slug: "brief-draft-14" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-14");
  });
  it("parseBriefIssueRow status draft 15", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 16, status: "draft", slug: "brief-draft-15" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-15");
  });
  it("parseBriefIssueRow status draft 16", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 17, status: "draft", slug: "brief-draft-16" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-16");
  });
  it("parseBriefIssueRow status draft 17", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 18, status: "draft", slug: "brief-draft-17" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-17");
  });
  it("parseBriefIssueRow status draft 18", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 19, status: "draft", slug: "brief-draft-18" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-18");
  });
  it("parseBriefIssueRow status draft 19", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 20, status: "draft", slug: "brief-draft-19" }));
    expect(issue?.status).toBe("draft");
    expect(issue?.slug).toBe("brief-draft-19");
  });
  it("parseBriefIssueRow status approved 0", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 1, status: "approved", slug: "brief-approved-0" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-0");
  });
  it("parseBriefIssueRow status approved 1", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 2, status: "approved", slug: "brief-approved-1" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-1");
  });
  it("parseBriefIssueRow status approved 2", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 3, status: "approved", slug: "brief-approved-2" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-2");
  });
  it("parseBriefIssueRow status approved 3", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 4, status: "approved", slug: "brief-approved-3" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-3");
  });
  it("parseBriefIssueRow status approved 4", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 5, status: "approved", slug: "brief-approved-4" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-4");
  });
  it("parseBriefIssueRow status approved 5", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 6, status: "approved", slug: "brief-approved-5" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-5");
  });
  it("parseBriefIssueRow status approved 6", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 7, status: "approved", slug: "brief-approved-6" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-6");
  });
  it("parseBriefIssueRow status approved 7", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 8, status: "approved", slug: "brief-approved-7" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-7");
  });
  it("parseBriefIssueRow status approved 8", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 9, status: "approved", slug: "brief-approved-8" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-8");
  });
  it("parseBriefIssueRow status approved 9", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 10, status: "approved", slug: "brief-approved-9" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-9");
  });
  it("parseBriefIssueRow status approved 10", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 11, status: "approved", slug: "brief-approved-10" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-10");
  });
  it("parseBriefIssueRow status approved 11", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 12, status: "approved", slug: "brief-approved-11" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-11");
  });
  it("parseBriefIssueRow status approved 12", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 13, status: "approved", slug: "brief-approved-12" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-12");
  });
  it("parseBriefIssueRow status approved 13", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 14, status: "approved", slug: "brief-approved-13" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-13");
  });
  it("parseBriefIssueRow status approved 14", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 15, status: "approved", slug: "brief-approved-14" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-14");
  });
  it("parseBriefIssueRow status approved 15", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 16, status: "approved", slug: "brief-approved-15" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-15");
  });
  it("parseBriefIssueRow status approved 16", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 17, status: "approved", slug: "brief-approved-16" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-16");
  });
  it("parseBriefIssueRow status approved 17", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 18, status: "approved", slug: "brief-approved-17" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-17");
  });
  it("parseBriefIssueRow status approved 18", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 19, status: "approved", slug: "brief-approved-18" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-18");
  });
  it("parseBriefIssueRow status approved 19", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 20, status: "approved", slug: "brief-approved-19" }));
    expect(issue?.status).toBe("approved");
    expect(issue?.slug).toBe("brief-approved-19");
  });
  it("parseBriefIssueRow status sent 0", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 1, status: "sent", slug: "brief-sent-0" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-0");
  });
  it("parseBriefIssueRow status sent 1", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 2, status: "sent", slug: "brief-sent-1" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-1");
  });
  it("parseBriefIssueRow status sent 2", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 3, status: "sent", slug: "brief-sent-2" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-2");
  });
  it("parseBriefIssueRow status sent 3", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 4, status: "sent", slug: "brief-sent-3" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-3");
  });
  it("parseBriefIssueRow status sent 4", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 5, status: "sent", slug: "brief-sent-4" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-4");
  });
  it("parseBriefIssueRow status sent 5", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 6, status: "sent", slug: "brief-sent-5" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-5");
  });
  it("parseBriefIssueRow status sent 6", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 7, status: "sent", slug: "brief-sent-6" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-6");
  });
  it("parseBriefIssueRow status sent 7", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 8, status: "sent", slug: "brief-sent-7" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-7");
  });
  it("parseBriefIssueRow status sent 8", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 9, status: "sent", slug: "brief-sent-8" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-8");
  });
  it("parseBriefIssueRow status sent 9", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 10, status: "sent", slug: "brief-sent-9" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-9");
  });
  it("parseBriefIssueRow status sent 10", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 11, status: "sent", slug: "brief-sent-10" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-10");
  });
  it("parseBriefIssueRow status sent 11", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 12, status: "sent", slug: "brief-sent-11" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-11");
  });
  it("parseBriefIssueRow status sent 12", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 13, status: "sent", slug: "brief-sent-12" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-12");
  });
  it("parseBriefIssueRow status sent 13", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 14, status: "sent", slug: "brief-sent-13" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-13");
  });
  it("parseBriefIssueRow status sent 14", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 15, status: "sent", slug: "brief-sent-14" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-14");
  });
  it("parseBriefIssueRow status sent 15", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 16, status: "sent", slug: "brief-sent-15" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-15");
  });
  it("parseBriefIssueRow status sent 16", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 17, status: "sent", slug: "brief-sent-16" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-16");
  });
  it("parseBriefIssueRow status sent 17", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 18, status: "sent", slug: "brief-sent-17" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-17");
  });
  it("parseBriefIssueRow status sent 18", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 19, status: "sent", slug: "brief-sent-18" }));
    expect(issue?.status).toBe("sent");
    expect(issue?.slug).toBe("brief-sent-18");
  });
  it("parseBriefIssueRow status sent 19", () => {
    const issue = parseBriefIssueRow(makeRow({ number: 20, status: "sent", slug: "brief-sent-19" }));
    expect(issue?.status).toBe("sent");
