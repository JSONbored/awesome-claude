...
describe("hasCompareItem", () => {
  it("matches by category + slug, not identity", () => {
    expect(hasCompareItem([A, B], { category: "agents", slug: "beta" })).toBe(true);
    expect(hasCompareItem([A, B], C)).toBe(false);
    // Same slug, different category must not match.
    expect(hasCompareItem([A], { category: "agents", slug: "alpha" })).toBe(false);
  });
...
describe("resolveCompareParam", () => {
  ...
  it("resolves refs to entries preserving param order", () => {
    expect(resolveCompareParam(entries, "commands/gamma,skills/alpha")).toEqual([C, A]);
  });

  it("trims whitespace and drops malformed / unmatched refs", () => {
    expect(
      resolveCompareParam(entries, " skills/alpha , not-a-ref , mcp/missing , agents/beta "),
    ).toEqual([A, B]);
  });

  it("de-duplicates repeated refs", () => {
    expect(resolveCompareParam(entries, "skills/alpha,skills/alpha")).toEqual([A]);
  });

  it("caps the result at the limit", () => {
    expect(
      resolveCompareParam(entries, "skills/alpha,agents/beta,commands/gamma,hooks/delta,mcp/epsilon"),
    ).toHaveLength(DEFAULT_COMPARE_LIMIT);
    expect(resolveCompareParam(entries, "skills/alpha,agents/beta,commands/gamma", 2)).toEqual([A, B]);
  });
});
